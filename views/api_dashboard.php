...
function getDateFilters() {
    ...
    if ($month !== null) {
        $whereClause .= " AND MONTH(FromDate) = :month";
        $params[':month'] = $month;
    }
    
    if ($day !== null) {
        $whereClause .= " AND DAY(FromDate) = :day";
        $params[':day'] = $day;
    }
...
function getSolicitudesPorEstado($conn) {
    ...
    $query = "SELECT s.status, COUNT(l.id) as total
            FROM tblleaves l
...
function getTendenciasPorTiempo($conn) {
    ...
    switch ($groupBy) {
        case 'month':
            $query = "SELECT 
                        MONTH(FromDate) as time_period,
                        COUNT(id) as total_solicitudes
                    FROM tblleaves
                    WHERE YEAR(FromDate) = :year
                    GROUP BY MONTH(FromDate)
                    ORDER BY MONTH(FromDate)";
            break;
        case 'quarter':
            $query = "SELECT 
                        QUARTER(FromDate) as time_period,
                        COUNT(id) as total_solicitudes
                    FROM tblleaves
                    WHERE YEAR(FromDate) = :year
                    GROUP BY QUARTER(FromDate)
                    ORDER BY QUARTER(FromDate)";
            break;
        case 'year':
            $query = "SELECT 
                        YEAR(FromDate) as time_period,
                        COUNT(id) as total_solicitudes
                    FROM tblleaves
                    GROUP BY YEAR(FromDate)
                    ORDER BY YEAR(FromDate)";
            $stmt = $conn->prepare($query);
            $stmt->execute();
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
            return $result;
...
function getEstadoDashboard($conn) {
    ...
    // Total solicitudes
    $query1 = "SELECT COUNT(id) as total_solicitudes FROM tblleaves WHERE 1=1 " . $dateFilters['whereClause'];
    ...
    $query2 = "SELECT SUM(TIMESTAMPDIFF(DAY, FromDate, ToDate) + 1) as total_dias 
               FROM tblleaves WHERE 1=1 " . $dateFilters['whereClause'];
    ...
    $query3 = "SELECT s.status, COUNT(l.id) as total
              FROM tblleaves l
              JOIN tblstatus s ON l.Status = s.id
              WHERE 1=1 " . $dateFilters['whereClause'] . "
              GROUP BY s.status";
